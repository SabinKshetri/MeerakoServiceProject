...
const AddBook = () => {
  ...
  const handSubmit = async (e) => {
    e.preventDefault();
    console.log(book);
    const respones = await Authenticated_API.post("/books", book);
    if (respones.status == 200 && respones.length != 0) {
      navigate("/book");
      toast.success(respones.data.message);
    } else {
      setErrorMessage(respones.data.message);
    }
        ...
        <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
          <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700 mt-11">
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form className="space-y-4 md:space-y-6" onSubmit={handSubmit}>
                <div>
                  <label
                    htmlFor="title"
